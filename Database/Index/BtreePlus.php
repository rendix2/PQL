...
class BtreePlus
{
    ...
    private function insertInternal($x, Node $cursor, Node $child) : void
    {
        if ($cursor->size < self::MAX) {
            $i = 0;
    ...
    public function searchKey($key, $value)
    {
        if ($this->root === null) {
            ...
            while ($cursor->isLeaf === false) {
                for ($i = 0; $i < $cursor->size; $i++) {
                    if (isset($cursor->key[$i]) && $value < $cursor->key[$i]->{$key}) {
                        $cursor = $cursor->ptr[$i];
            ...
            for ($i = 0; $i < $cursor->size; $i++) {
                if (isset($cursor->key[$i]) && $cursor->key[$i]->{$key} === $value) {
                    return $cursor->key[$i];
                }
            }
    ...
    private function deleteEntry($x)
    {
        ...
        $neighbor = $neighborIndex === -1 ? $parent->ptr[1] : $parent->ptr[$neighborIndex];
    ...
    private function coalesceNodes(Node $root, Node $n, Node $neighbor = null, $neighborIndex, $kPrime)
    {
        if ($neighborIndex === -1) {
            $tmp = $n;
            $n = $neighbor;
        ...
        if (!$n->isLeaf) {
            ...
            $nStart = 0;
            if ($n->size + $neighbor->size >= static::MAX) {
                $split = true;
                $nEnd = $this->cut(static::MAX) - 2;
            }
            ...
            if ($split) {
                $newKPrime = $n->key[$nStart];

                for ($i = 0, $j = $nStart + 1; $i < $n->size; $i++, $j++) {
                    $n->key[$i] = $n->key[$j];
                    $n->ptr[$i] = $n->ptr[$j];
                }
